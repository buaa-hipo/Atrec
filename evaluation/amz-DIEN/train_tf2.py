...
class DIEN():
    def __init__(self,
        ...
        self.is_training = not saved_model
        ...
        if self.is_structured:
            ...
            create_split_ops(self._feature['ITEM'])
            create_split_ops(self._feature['CATEGORY'])
    ...
    def _create_model(self):
        ...
        with tf.variable_scope('rnn_1'):
            ...
            # Create tile ops
            if self.is_structured:
                run_output_1 = create_tile_ops(run_output_1, ITEM_PACKSIZE)
                his_item_emb = create_tile_ops(his_item_emb, ITEM_PACKSIZE)
                noclk_his_item_emb = create_tile_ops(noclk_his_item_emb, ITEM_PACKSIZE)
                sequence_length = create_tile_ops(sequence_length, ITEM_PACKSIZE)
            mask = tf.sequence_mask(sequence_length)
        # Aux loss
        ...
        # Create tiled ops
        if self.is_structured:
            uid_emb_tiled = create_tile_ops(uid_emb, ITEM_PACKSIZE)
            item_his_eb_sum_tiled = create_tile_ops(item_his_eb_sum, ITEM_PACKSIZE)
...
def main(tf_config=None, server=None):
    ...
    if is_saved_model:
        with tf.Session(config=sess_config) as sess:
            ...
            # if True:
            #     return
            # timeline profiling for serving model
            run_options = None
            if True:
            ...
            # l = 0
            # feature_names = ["UID", "ITEM", "CATEGORY", "HISTORY_ITEM", "HISTORY_CATEGORY"]
            # neg_feature_names = ["NOCLK_HISTORY_ITEM", "NOCLK_HISTORY_CATEGORY"]
            # all_feature_names = feature_names + neg_feature_names
            # NUM_USER = 1
            # f_dict = {}
            # request_file = 'requests'
            # if args.structured:
            #     request_file = 'requests_packed'
            ...
            #                 f_dict[real_input[name]] =[]
            #         for i in range(0, len(all_feature_names)):
            #             f_dict[real_input[all_feature_names[i]]].append(bytes(items[i], 'utf-8'))
            #         l = l + 1
            # sess.run(tf.tables_initializer())
            # op = model._feature["ITEM"].consumers()[0]
            # op = op.outputs[0].consumers()[0]
            # op = op.outputs[0].consumers()[0]
            # print(sess.run(op.outputs, feed_dict=f_dict, options=run_options, run_metadata=run_metadata))
            # sess.run([model.output], feed_dict=f_dict, options=run_options, run_metadata=run_metadata)
            # tl = timeline.Timeline(run_metadata.step_stats)
            # with open('serve_timeline.json', "w") as f:
